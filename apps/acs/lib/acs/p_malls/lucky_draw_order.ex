defmodule Acs.PMalls.LuckyDrawOrder do
  ...
  @derive {Poison.Encoder, except: [:app, :wcs_user, :lucky_draw :__meta__]}
  ...
  def changeset(%LuckyDrawOrder{} = lucky_draw_order, attrs) do
    lucky_draw_order
    |> cast(attrs, [:name, :pic, :status, :app_id, :wcs_user_id, :lucky_draw_id, :address, :paid_at])
    |> validate_required([:name, :status, :app_id, :wcs_user_id, :lucky_draw_id])
  end
end
