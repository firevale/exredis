defmodule Acs.Wcp.AppWcpTFDownloadResponse do
  ...
  def is_waiting_email(app_id, openid) do 
    Exredis.exists("_wcp.tfd_wait_email.#{app_id}.#{openid}") == 1
  end

  def build_email_reply(app_id, openid, email) do 
    ...
    do 
      if Regex.match?(~r/\A([\w+\-].?)+@[a-z\d\-]+(\.[a-z]+)*\.[a-z]+\z/i, email) do 
        case Wcp.get_app_wcp_user(app_id, email: email) do 
          %AppWcpUser{openid: ^openid, tf_email: ^email} -> 
            String.replace(cfg.tf_already_invited_template, "#email", email)

          nil ->
            _invite_user(app_id, openid, email)
  ...
  defp _invite_user(app_id, openid, email) do 
    Process.spawn(fn ->
      ...
      do 
        if TestFlight.is_tester?(itc_app_id, email) do 
          ...
          Exwcp.Message.Custom.send_text(cfg, openid, content)  
        else 
          case Wcp.get_app_wcp_user(app_id, openid: openid) do 
            nil ->
              error "invite tf tester error:  can't find app wcp user, app_id: #{app_id}, openid: #{openid}"
              :ok 

            %AppWcpUser{openid: ^openid, tf_email: nil, nickname: nickname} = wcp_user ->
              case TestFlight.add_tester!(itc_app_id, email, nickname) do 
                {:ok, _status, _num_testers} ->
                  Exredis.del("_wcp.tfd_wait_email.#{app_id}.#{openid}")
            ...
            %AppWcpUser{openid: ^openid, tf_email: old_email, nickname: nickname} = wcp_user ->
