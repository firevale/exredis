defmodule AcsWeb.Admin.ForumController do
  ...
  def update_forum_icon(conn, %{"forum_id" => forum_id, "file" => %{path: image_file_path}} = _params) do
    case Forums.get_forum(forum_id) do
      ...
      %Forum{} = forum ->
        ...
        Forums.update_forum(forum, %{icon: icon_path})
  ...
  # update_forum
  def update_forum(%Plug.Conn{private: %{acs_admin_id: acs_admin_id, acs_app_id: app_id}} = conn, 
    %{"forum" => %{"id" => forum_id} = forum_info}) do
    case Forums.update_forum(forum_id, forum_info) do
      nil ->
        conn |> json(%{success: false, i18n_message: "error.server.forumNotFound"})

      {:ok, changes} ->
        Admin.log_admin_operation(acs_admin_id, app_id, "update_forum", changes)
        conn |> json(%{success: true, i18n_message: "admin.serverSuccess.forumUpdated"})
    end
  end

  # update_forum_section
  def update_forum_section(conn, %{"section" => %{"title" => ""}}) do
    conn |> json(%{success: false, i18n_message: "error.server.emptySectionTitle"})
  end
  def update_forum_section(conn, %{"section" => %{"forum_id" => ""}}) do
    conn |> json(%{success: false, i18n_message: "error.server.emptyForumId"})
  end
  def update_forum_section(%Plug.Conn{private: %{acs_admin_id: acs_admin_id, acs_app_id: app_id}} = conn, 
                          %{"section" => %{
                          "id" => _id,
                          "title" => _title,
                          "sort" => _sort,
    ...
    case Forums.update_forum_section(section) do
      {:ok, new_section, changes} ->
        Admin.log_admin_operation(acs_admin_id, app_id, "update_forum_section", changes)
        conn |> json(%{success: true, section: new_section })

      {:ok, new_section} ->
        Admin.log_admin_operation(acs_admin_id, app_id, "update_forum_section", section)
        conn |> json(%{success: true, section: new_section })
