defmodule TcpServer do
  ...
  def start_link(port, opts, module, sup \\ TcpConn.Supervisor) do
    GenServer.start_link(__MODULE__, [port, opts, module, sup])
  end

  def init([port, opts, module, sup]) do
    ...
    do
      {:ok, %{socket: socket, acceptor: acceptor, conn_counter: 0, module: module, sup: sup}}
    else 
      _ ->
  ...
  def handle_info({:inet_async, listen_socket, _acceptor, {:ok, client_socket}}, %{conn_counter: counter} = state) do 
    try do 
      with :ok <- set_sockopt(listen_socket, client_socket),
           :ok <- start_conn(client_socket, state.module, state.sup),
  ...
  defp start_conn(client_socket, module, sup) do 
    with {:ok, pid} <- Supervisor.start_child(sup, []),
