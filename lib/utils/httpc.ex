defmodule Utils.Httpc do
  ...
  def post_msg(url, %{} = msg) do 
    Logger.debug "http post: #{url} msg: #{msg |> URI.encode_query}"
    post url, body: msg |> URI.encode_query, headers: ["Content-Type": "application/x-www-form-urlencoded"], timeout: 30_000
  end

  def post_json(url, %{} = msg) do 
    Logger.debug "http post: #{url} msg: #{msg |> JSON.encode!}"
    post url, body: msg |> JSON.encode!, headers: ["Content-Type": "application/json"]
  end
