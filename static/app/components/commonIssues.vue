  ...
  <div v-show="!issueDetail" class="content-item">
    <div class="row-menu is-child content-item">
      <div style="display: flex; flex: 3;position: relative;">
        <input class="search-input dark" v-model="issueKey" type="text" :placeholder="$t('customerService.searchPlaceHolder')"
      ...
      <input class="search-button" type="button" :value="$t('customerService.searchBtn')" @click="searchIssueByKey(issueKey)"></input>
    </div>
...
export default {
  ...
  computed: {
    ...
    issueTitle() {
      return this.$t('customerService.issueTitle', {
