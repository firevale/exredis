...
<nav v-show="pageCount > 1" class="pagination">
  <ul>
    <li> <a class="button" @click.prevent="selectPage(currentPage - 1)" :disabled="currentPage == 1"  :class="{'is-disabled': currentPage == 1}">{{ $t('forum.pagination.previous') }}</a>      </li>
    ...
    <li> <a class="button" :disabled="currentPage == pageCount" :class="{'is-disabled': currentPage == pageCount}" @click.prevent="selectPage(currentPage + 1)">{{ $t('forum.pagination.next') }}</a>        </li>
  </ul>
</nav>
</template>
