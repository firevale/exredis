  ...
  IScroll.prototype = {
    ...
    _end: function(e) {
      ...
      if (this.options.momentum && duration < 300) {
        momentumX = this.hasHorizontalScroll ? utils.momentum(this.x, this.startX, duration,
          this.maxScrollX, this.options.bounce ? this.wrapperWidth : 0, this.options.deceleration
        ) : { destination: newX, duration: 0 };
        momentumY = this.hasVerticalScroll ? utils.momentum(this.y, this.startY, duration,
    ...
    refresh: function() {
      ...
      var limit;
      if (this.options.infiniteElements) {
        this.options.infiniteLimit = this.options.infiniteLimit || Math.floor(2147483645 /
          this.infiniteElementHeight);
    ...
    scrollToElement: function(el, time, offsetX, offsetY, easing) {
      ...
      time = time === undefined || time === null || time === 'auto' ? Math.max(Math.abs(this.x -
        pos.left), Math.abs(this.y - pos.top)) : time;
    ...
    _initInfinite: function() {
      ...
      this.on('refresh', function() {
        ...
        this.infiniteUpperBufferSize = Math.floor((this.infiniteLength - elementsPerPage) /
          2);
    ...
    reorderInfinite: function() {
      ...
      var minorPhase = Math.max(Math.floor(-this.y / this.infiniteElementHeight) - this.infiniteUpperBufferSize,
          0),
        ...
        if (this.infiniteElements[i]._top !== top) {
          ...
          if (this.infiniteElements[i]._phase < this.options.infiniteLimit) {
            ...
            if (this.options.infiniteUseTransform) {
              this.infiniteElements[i].style[utils.style.transform] = 'translate(0, ' + top +
                'px)' + this.translateZ;
            } else {
      ...
      if (this.cachePhase != cachePhase && (cachePhase === 0 || minorPhase - this.infiniteCacheBuffer >
          0)) {
        this.options.dataset.call(this, Math.max(cachePhase * this.infiniteCacheBuffer - this
          .infiniteCacheBuffer, 0), this.options.cacheSize);
      }
